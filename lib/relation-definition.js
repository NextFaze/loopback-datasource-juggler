...
HasMany.prototype.updateById = function(fkId, data, options, cb) {
  ...
  this.findById(fkId, options, function(err, inst) {
    ...
    // HACK: When using postgres-connector with guid attempting to update the
    //       attributes of a related model, e.g. PUT /api/client/:id/note/:id,
    //       causes a new id to be created, which the update rightfully rejects.
    //       Until we can figure out where the id is getting generated this
    //       hack, copying the correct id to the update data, fixes the problem.
    //
    //       Doing a search for '.updateAttributes' turns up a number of other
    //       cases where there appears to be id handling/mutating before
    //       '.updateAttributes' is called. It may be just a case of that being
    //       missed here.
    if(data.id && data.id !== inst.id) {
      data.id = inst.id;
    }
    // end hack
